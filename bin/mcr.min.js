...
$('nav#'+panel).toggle('fast','linear');}}},Get:{chapter:function(){MCR.Do.displayStatus('Loading Ch. '+MCR.Global.request.chapter+'...');(new Chapter(MCR.Global.request.manga,MCR.Global.request.chapter)).get();},updateDisplay:function(e,pages){console.log('Hey');var i,h="",t;for(i in pages){var tt=MCR.Global.manga.title+' / '+MCR.Global.request.chapter+': '+MCR.Global.manga.chapter['name']+' / '+'Page '+page;var ss=MCR.Option.switched('spacing')?'margin-bottom: 1em;':'margin-bottom: 0px;';h+="<li><img src='"+pages[page]+"' alt='"+tt+"' title='"+tt+"' style='"+ss+"' /></li>\n";}
...
var c,html="";for(var i in MCR.Global.manga.chapters){c=MCR.Global.manga.chapter.index==i?' selected="selected" ':' ';html+="<option value='"+MCR.Global.manga.chapters[i]['chapter']+"'"+c+">"+MCR.Global.manga.chapters[i]['chapter']+"\n";}
$('#chapters').html(html);},selectedChapter:function(){MCR.Global.request.chapter=$('#chapters').val();MCR.Get.chapter();},previousChapter:function(){var chi=Number(MCR.Global.manga.chapter.index);if(MCR.Global.manga.chapters[chi-1]){MCR.Global.request.chapter=MCR.Global.manga.chapters[chi-1]['chapter'];MCR.Get.chapter();}else{MCR.Do.displayStatus('This is the first Chapter');}},nextChapter:function(){var chi=Number(MCR.Global.manga.chapter.index);if(MCR.Global.manga.chapters[chi+1]){MCR.Global.request.chapter=MCR.Global.manga.chapters[chi+1]['chapter'];MCR.Get.chapter();}else{MCR.Do.displayStatus('This is the last Chapter');}},init:function(){var $W=$(window);$W.bind(Chapter.ep,function(){MCR.Do.displayStatus('Loading Ch. '+MCR.Global.request.chapter+'... ('+page+')',false);});$W.bind(Chapter.ec,MCR.Get.updateDisplay);}}};window.MCR=MCR;})(window);$(function(){function omvKeyPressed(){}
document.onkeydown=function(){};MCR.Global.request=MCR.Tool.parseUrl(window.location);MCR.Global.manga.title=MCR.Tool.capitalize(MCR.Global.request.manga.replace(/-/ig,' '));MCR.Global.manga.chapter={"number":MCR.Global.request.chapter,"name":""};MCR.UI.init();MCR.Option.init();MCR.Do.panel.init();MCR.Get.init();MCR.Info.show();MCR.Get.chapter();console.log('Hey',Chapter.ep);$(window).bind("popstate",function(){MCR.Global.request=MCR.Tool.parseUrl(window.location);MCR.Get.chapter();});});