...
MCR.Option.switchOff(key);$('#option-'+key).attr('checked',false);}},values:{}},Do:{displayStatus:function(status,animate){if(typeof animate!=='boolean'){animate=true;}
if(animate){$('#status').stop().css('opacity','1.0').fadeOut('fast',function(){$('#status').text(status).fadeIn();});}else{$('#status').stop().text(status).show();}
if(status.length>0){status+=' / ';}
document.title=status+MCR.Global.manga.title+' / Ch. '+MCR.Global.request.chapter;},handleHotkeys:function(e){switch(e.which){case 188:MCR.Get.previousChapter();return;case 190:MCR.Get.nextChapter();return;default:return;}},toggle:{hotkeys:function(force_to){MCR.Option.toggle('hotkeys',force_to,function(){$(document).bind('keydown',MCR.Do.handleHotkeys);},function(){$(document).unbind('keydown',MCR.Do.handleHotkeys);});},spacing:function(force_to){MCR.Option.toggle('spacing',force_to,function(){$('article').addClass('spaced');},function(){$('article').removeClass('spaced');});},horizontal:function(force_to){MCR.Option.toggle('horizontal',force_to,function(){$('article').addClass('horizontal');MCR.TMP.width=0;$('article img').each(function(){var w=$(this).width();if(w<800){w=800;}
MCR.TMP.width+=w+20;});$('article ul').css('width',MCR.TMP.width);},function(){$('article').removeClass('horizontal');$('article ul').css('width','');});},black:function(force_to){var a=$('[class*=white], [class*=black]');MCR.Option.toggle('black',force_to,function(){a.addClass('black').removeClass('white');},function(){a.addClass('white').removeClass('black');});},forced800:function(force_to){MCR.Option.toggle('forced800',force_to,function(){$('article').addClass('forced800');},function(){$('article').removeClass('forced800');});},ads:function(force_to){MCR.Option.toggle('ads',force_to,function(){$('#ads').html("    <iframe width='350' height='250' frameborder='no' scrolling='no' src='http://ad.mangareader.net/btleft' class='left' />"+"    <iframe width='350' height='250' frameborder='no' scrolling='no' src='http://ad.mangareader.net/btright' class='right' />").show();},function(){$('#ads').html("").hide();});}},panel:{init:function(){$('#options-button').click(MCR.Do.panel.toggle);$('#info-button').click(function(){MCR.Do.panel.toggle('info');});$('select#chapters').live('change',MCR.Get.selectedChapter);$('#prev').click(MCR.Get.previousChapter);$('#next').click(MCR.Get.nextChapter);$('#option-spacing').change(MCR.Do.toggle.spacing);$('#option-horizontal').change(MCR.Do.toggle.horizontal);$('#option-black').change(MCR.Do.toggle.black);$('#option-hotkeys').change(MCR.Do.toggle.hotkeys);$('#option-forced800').change(MCR.Do.toggle.forced800);$('#option-ads').change(MCR.Do.toggle.ads);MCR.Do.toggle.spacing(MCR.Option.switched('spacing'));MCR.Do.toggle.horizontal(MCR.Option.switched('horizontal'));MCR.Do.toggle.black(MCR.Option.switched('black'));MCR.Do.toggle.hotkeys(MCR.Option.switched('hotkeys'));MCR.Do.toggle.forced800(MCR.Option.switched('forced800'));MCR.Do.toggle.ads(MCR.Option.switched('ads'));},show:function(panel){if(typeof panel!=='string'){panel='options';}
