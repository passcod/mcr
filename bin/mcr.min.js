...
MCR.Global.request.chapter+': '+MCR.Global.manga.chapter['name']+' / '+'Page '+page;return"<img src='"+MCR.TMP.list[page]+"' alt='"+t+"' title='"+t+"' />\n";},updateDisplay=function(use_cache){var i,h="",t;for(i in MCR.TMP.list){h+=imgTag(i);}
$('article').html(h);MCR.Do.displayStatus('Loaded');if(MCR.Option.switched("cache")){var c=MCR.Cache.get('chapters');if(!c[String(MCR.Global.request.manga)]){c[String(MCR.Global.request.manga)]={};}
c[String(MCR.Global.request.manga)][String(MCR.Global.request.chapter)]=MCR.TMP.list;MCR.Cache.set('chapters',c);}},doPage1=function(doc,cached){if(cached){}else{$('article').html(imgTag(0));}
