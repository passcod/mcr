...
self.loadChapter();};this.setChapter=function(n){chapter=n-1;};this.getChapter=function(){return chapter;};this.loadChapter=function(n){var chn=n||chapter;chapters[numbers[chn]].data.get();};var request=function(url,callback){$.ajax({type:"GET",url:url,success:function(data){callback((new DOMParser()).parseFromString(data,"text/xml"));},error:function(){callback((new DOMParser()).parseFromString("<html><head></head><body></body></html>","text/xml"));}});};};window.Ç.Manga=Manga;})(window);"use strict";(function(window){var Ui=function(){var manga=title=chapterno=chaptername=artist=author=release=rstatus="";var navs=['info','options','hotkeys','theme','favs','mark'];var buttons=['previous','next','reload','home','info','options','permalink','hotkeys','theme','favs','mark'];var hotkeys={'next':68,'previous':65,'home':87,'reload':83,'info':73,'options':79,'hotkeys':72,'theme':84,'favs':70,'mark':66,'theme1':49,'theme2':50,'theme3':51,'favadd':-1,'favdel':-1,'favtog':-1,'markadd':-1,'markdel':-1,'marktog':-1};var self=this;var k;this.setStatus=function(message){message=message||'';var msg=message+(message==''?'':' / ');$('#status').text(message);$('title').text(msg+chapterno+chaptername+" / "+title);};this.updateURL=function(){if(Modernizr.history){history.pushState({manga:manga,chapter:chapterno},$('title').text(),"http://www.mangareader.net/"+manga+"/"+chapterno);}
$('#button-permalink').attr('href',"http://www.mangareader.net/"+manga+"/"+chapterno);};this.setInfo=function(_manga,_chapterno,_chaptername,_artist,_author,_release,_rstatus){function def(val,def){if(val===""||val===false){return def;}
return val;}
manga=def(_manga,manga);chapterno=def(_chapterno+1,chapterno);chaptername=def(_chaptername,chaptername);artist=def(_artist,artist);author=def(_author,author);release=def(_release,release);rstatus=def(_rstatus,rstatus);title=manga.replace(/-/g,' ').replace(/^([a-z])|\s+([a-z])/g,function($1){return $1.toUpperCase();});if(chaptername!==""){chaptername=": "+chaptername;}
var $info=$('nav#info');$('h1',$info).text(title);$('h2',$info).text(chapterno+chaptername);$('date',$info).text(release);$('#stat',$info).text(rstatus);if(author==artist){$('#arthor').html('Story / Art by <strong>'+author+'</strong>.');}else{if(artist==''){artist=author;}
if(artist==''){$('#arthor').html('Author / Artist unknown.');}else{if(author==''){author=artist;}
$('#arthor').html('Story / Art by <strong>'+author+'</strong> / <strong>'+artist+'</strong>.');}}};this.addOption=function(name,description,default_state){$('nav#options').append('<p id="option-'+name+'"><label><input type="checkbox" /> '+description+'</label></p>');if(default_state===true)$('#option-'+name+' input').attr('checked','checked');hotkeys['opt'+name]=-1;};this.setOption=function(name,state){$('#option-'+name+' input').removeAttr('checked');if(state)$('#option-'+name+' input').attr('checked','checked');};this.getOption=function(name){return $('#option-'+name+' input').is(':checked');};this.showMain=function(){$('nav#main').clearQueue().animate({top:'0px'},'fast');};this.hideMain=function(){if($('nav#main').data('let')===false){$('div#mask').fadeOut();$('nav#main').clearQueue().animate({top:'-36px'},'fast');}};this.showNav=function(name){for(n in navs){if(navs[n]!=name){this.hideNav(navs[n]);}}
...
this.setHotkey=function(name,key){hotkeys[name]=key;};this.changeColours=function(background,text,border){function colorStyle(color){var colour;if(Modernizr.rgba){colour='rgba('+color[0]+', '+color[1]+', '+color[2]+', '+color[3]+')';}else{colour='rgb('+color[0]+', '+color[1]+', '+color[2]+')';}
return colour;}
var ruls={columnRule:colorStyle(border)};ruls[Modernizr.prefixed('columnRuleColor')]=colorStyle(border);$('.has-bg').css('background-color',colorStyle(background));$('.has-border').css('border-color',colorStyle(border));$('.has-rule').css(ruls);$('body').css('color',colorStyle(text));};this.display=function(pages){var p,$p=$('article ul');$('li',$p).remove();for(var p in pages){$p.append('<li><img src="'+pages[p]+'" /></li>');}};$('head').append('<style>'+Ç.data.css+'</style>');$('body > *').not('script, #container').remove();$('body').append(Ç.data.html).add('header nav').addClass('has-bg');$('header nav').addClass('has-border');$('#version').text(Ç.data.version);$('nav#main').hover(function(){self.showMain();$(this).data('hover',true);},function(){self.hideMain();$(this).data('hover',false);}).data('hover',false);for(k in navs){(function(name,that){$(window).bind("key"+name+" hit"+name,function(){if($('nav#'+name).is(':hidden')){that.showNav(name);that.showMain();}else{that.hideNav(name);if($('nav#main').data('hover')===false){that.hideMain();}}});})(navs[k],this);}
for(k in buttons){(function(name){$('#button-'+name).click(function(){$(window).trigger("hit"+name);});})(buttons[k]);$('#button-'+buttons[k]+' img').attr('src','data:image/png;base64,'+Ç.data[buttons[k]]);}
...
document.onkeydown=function(){};var options={"spacing":{"description":"Add spacing between pages.","default":true,"able":true},"horizontal":{"description":"Horizontal reading. Minimum comfortable screen height 768px, recommended 1200px.","default":false,"able":true},"reverse":{"description":"Reverse the order of the pages. Can provide a more natural reading experience, especially in horizontal mode.","default":false,"able":true},"forced800":{"description":"Force image width to 800. Provides a more uniform reading experience (in vertical mode), but you might want to disable it if the text is too small to read. In horizontal mode, it is better to disable it.","default":false,"able":true},"fullwidth":{"description":"Allows the nav/panels to extends to the full width of the screen.","default":true,"able":true},"history":{"description":"Update the URL when changing chapter."+(Modernizr.history?'':' Requires a modern browser.'),"default":Modernizr.history,"able":Modernizr.history},"preload":{"description":"Preload the next chapter as you are reading this one."+(Modernizr.postmessage?'':' Requires a modern browser.'),"default":false,"able":Modernizr.postmessage}};var parseURL=function(){var U={},url_str=window.location.href,Ro=/mangareader\.net\/[0-9]+\-[0-9]+\-([0-9]+)\/([a-z0-9\-]+)\/chapter-([0-9]+)\.html?/i.exec(url_str),Rn=/mangareader\.net\/([a-z0-9\-]+)(\/[0-9]+)?(\/[0-9]+)?/i.exec(url_str),ss=/[\/]/;if(Ro!==null){U.page=Ro[1]?Number(Ro[1].replace(ss,'')):null;U.manga=Ro[2]?Ro[2].replace(ss,''):null;U.chapter=Ro[3]?Number(Ro[3].replace(ss,'')):null;}else if(Rn!==null){U.manga=Rn[1]?Rn[1].replace(ss,''):null;U.chapter=Rn[2]?Number(Rn[2].replace(ss,'')):null;U.page=Rn[3]?Number(Rn[3].replace(ss,'')):null;}else{return null;}
...
ui.display(pages);if(gotinfo){ui.setInfo(false,manga.getChapter(),manga.chapters()[manga.getChapter()].name);ui.setStatus('');ui.updateURL();}}).bind("gotinfo",function(e,page){manga.addChapter(c);gotinfo=true;ui.setInfo(manga.info().name,manga.getChapter(),manga.chapters()[manga.getChapter()].name,manga.info().artist,manga.info().author,manga.info().pubdate,manga.info().status);ui.setStatus('');ui.updateURL();}).bind("gotpage",function(e,page,chap){ui.setStatus('Loading Ch. '+chap.num()+' ('+page+')');}).bind("firstchapter",function(){ui.setStatus('This is the first chapter');}).bind("lastchapter",function(){ui.setStatus('This is the last chapter');}).bind("keyreload hitreload",function(){window.location=$('#button-permalink').attr('href');}).bind("keyhome hithome",function(){window.location="http://www.mangareader.net";}).bind("keyprevious hitprevious",function(){manga.previousChapter();}).bind("keynext hitnext",function(){manga.nextChapter();}).bind("popstate",function(){info=parseURL();if(manga!==-1){manga.setChapter(info.chapter);manga.loadChapter();}});ui.changeColours([255,255,255,1],[0,0,0,1],[199,228,64,0.8]);for(var n in options){ui.addOption(n,options[n]['description'],options[n]['default']);}
ui.setStatus('Ready');});