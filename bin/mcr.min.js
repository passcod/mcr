...
var MCR_VERSION='11.30';if(!this.JSON){this.JSON={};}
...
return len;}else{return 0;}}},Option:{defaults:{"spacing":"1em","allblack":"off","hotkeys":"off","showfirst":"off","forced800":"on","ads":"on","cache":"off","cache-images":"{}"},init:function(){var key;for(key in MCR.Option.defaults){if(MCR.Tool.canHazStorage()){MCR.Option.values[key]=window.localStorage[MCR.Info.keyPrefix+key]?window.localStorage[MCR.Info.keyPrefix+key]:MCR.Option.defaults[key];window.localStorage[MCR.Info.keyPrefix+key]=MCR.Option.values[key];}else{MCR.Option.values[key]=MCR.Option.defaults[key];}}},set:function(key,value){MCR.Option[key]=value;if(MCR.Tool.canHazStorage()){window.localStorage[MCR.Info.keyPrefix+key]=MCR.Option[key];$('#control-cachesize span').text(MCR.Tool.storageUsed(true));}},get:function(key){var value=MCR.Option[key];if(MCR.Tool.canHazStorage()){value=window.localStorage[MCR.Info.keyPrefix+key];}
...
MCR.Option.switchOn(key);$('#legend-'+key+' .control-now').text('On');}else{if(callOff){callOff();}
MCR.Option.switchOff(key);$('#legend-'+key+' .control-now').text('Off');}},values:{}},Cache:{init:function(){if(MCR.Option.switched('cache')){MCR.Cache.store.chapters=MCR.Cache.get('chapters');}},set:function(key,value){var json=JSON.stringify(value);MCR.Option.set('cache-'+key,json);},get:function(key){var json=MCR.Option.get('cache-'+key);return JSON.parse(json);},d:function(key,query){return false;},clear:function(){for(i in MCR.Cache.store){MCR.Cache.set(i,{});}},store:{chapters:{}}},Do:{displayStatus:function(status,animate){if(typeof animate!=='boolean'){animate=true;}
if(animate){$('#status').stop().fadeOut('fast',function(){$('#status').text(status).fadeIn();});}else{$('#status').stop().text(status).show();}
document.title=MCR.Global.manga.title+' / '+MCR.Global.request.chapter+' / '+status;},change:{spacing:function(force_to){if(/[0-9]+(em|px)/i.test(force_to)){$('#pages img').css('margin-bottom',force_to);}else{var s=$('#control-spacing input').val();MCR.Option.set('spacing',s);$('#pages img').css('margin-bottom',s);}
$('#legend-spacing .control-now').text(MCR.Option.get('spacing'));}},handleHotkeys:function(e){switch(e.which){case 188:MCR.Get.previousChapter();return;case 190:MCR.Get.nextChapter();return;default:return;}},toggle:{hotkeys:function(force_to){MCR.Option.toggle('hotkeys',force_to,function(){$(document).bind('keydown',MCR.Do.handleHotkeys);},function(){$(document).unbind('keydown',MCR.Do.handleHotkeys);});},allBlack:function(force_to){var a=$('[class*=white], [class*=black]');MCR.Option.toggle('allblack',force_to,function(){a.addClass('black').removeClass('white');},function(){a.addClass('white').removeClass('black');});},showFirst:function(force_to){MCR.Option.toggle('showfirst',force_to);},cache:function(force_to){MCR.Option.toggle('cache',force_to);},forced800:function(force_to){MCR.Option.toggle('forced800',force_to,function(){$('#pages').addClass('forced800');},function(){$('#pages').removeClass('forced800');});},ads:function(force_to){MCR.Option.toggle('ads',force_to,function(){$('#adfooter').show();},function(){$('#adfooter').hide();});}},panel:{init:function(){$('#options-button').click(MCR.Do.panel.toggle);$('#chapters select').live('change',MCR.Get.selectedChapter);$('#previous').click(MCR.Get.previousChapter);$('#next').click(MCR.Get.nextChapter);$('#cache-button').click(function(){MCR.Do.panel.toggle('cache');});$('#info-button').click(function(){MCR.Do.panel.toggle('info');});MCR.Do.change.spacing(MCR.Option.get('spacing'));MCR.Do.toggle.allBlack(MCR.Option.switched('allblack'));MCR.Do.toggle.hotkeys(MCR.Option.switched('hotkeys'));MCR.Do.toggle.showFirst(MCR.Option.switched('showfirst'));MCR.Do.toggle.cache(MCR.Option.switched('cache'));MCR.Do.toggle.forced800(MCR.Option.switched('forced800'));MCR.Do.toggle.ads(MCR.Option.switched('ads'));},show:function(panel){if(typeof panel!=='string'){panel='options';}
...
$('nav#'+panel).toggle('fast','linear');}}},Get:{chapter:function(){var url,i,makeUrl=function(){return MCR.Tool.buildUrl(MCR.Global.request.manga,MCR.Global.request.chapter,MCR.TMP.page);},updateSelect=function(doc){},updateDisplay=function(use_cache){var i,h="",t;for(i in MCR.TMP.list){t=MCR.Global.manga.title+'/'+
MCR.Global.request.chapter+': '+MCR.Global.manga.chapter['name']+'/'+'Page '+i;h+="<img src='"+MCR.TMP.list[i]+"' alt='"+i+"' title='"+i+"' />\n";}
$('article').html(h);MCR.Do.displayStatus('Loaded');},doPage1=function(doc){MCR.Global.manga.chapter['number']=MCR.Global.request.chapter;var sss,ss,s=$('script:last',doc).contents().text();s=s.split("\n");ss=s.indexOf("function omvKeyPressed(e) {");sss=s.splice(0,ss).join("\n");eval(sss);MCR.Global.manga.id=document['mangaid'];$.getJSON('/actions/selector/?id='+MCR.Global.manga.id+'&which=0',function(j){updateSelect(j);var i,k;for(i in j){k=j[i];if(k.chapter==String(MCR.Global.request.chapter)){MCR.Global.manga.chapter['name']=j[i]['chapter_name'];}
