...
url+"</div>";if(stat=='timeout'){data+='<img src="/qwertyuiopasdfghjklzxcvbnm.404" alt="-" width="800" />';}
...
$('nav#'+panel).toggle('fast','linear');}}},Get:{chapter:function(){var url,i,makeUrl=function(){return MCR.Tool.buildUrl(MCR.Global.request.manga,MCR.Global.request.chapter,MCR.TMP.page);},updateSelect=function(chapter_list){},imgTag=function(page){var t=MCR.Global.manga.title+'/'+
...
$('article').html(h);MCR.Do.displayStatus('Loaded');},doPage1=function(doc){$('article').html(imgTag(0));MCR.Global.manga.chapter['number']=MCR.Global.request.chapter;var sss,ss,s=$('script:last',doc).contents().text();s=s.split("\n");ss=s.indexOf("function omvKeyPressed(e) {");sss=s.splice(0,ss).join("\n");eval(sss);MCR.Global.manga.id=document['mangaid'];$.getJSON('/actions/selector/?id='+MCR.Global.manga.id+'&which=0',function(j){updateSelect(j);var i,k;for(i in j){k=j[i];if(k.chapter==String(MCR.Global.request.chapter)){MCR.Global.manga.chapter['name']=j[i]['chapter_name'];}
delete k['chapter'];MCR.Global.manga.chapters[j[i]['chapter']]=k;}
MCR.Info.show();});MCR.Tool.getFake(MCR.Tool.buildUrl(MCR.Global.request.manga),function(doc){MCR.Global.manga.description=$('#readmangasum p',doc).text();MCR.Global.manga.cover=$('#mangaimg img',doc).attr('src');MCR.Global.manga.pubdate=$('#mangaproperties tr:eq(2) td:last',doc).text();MCR.Global.manga.status=$('#mangaproperties tr:eq(3) td:last',doc).text();MCR.Global.manga.author=$('#mangaproperties tr:eq(4) td:last',doc).text();MCR.Global.manga.artist=$('#mangaproperties tr:eq(5) td:last',doc).text();MCR.Info.show();});},mainLoop=function(){MCR.Do.displayStatus('Loading Ch. '+MCR.Global.request.chapter+'...'+'('+MCR.TMP.page+')',false);MCR.Tool.getFake(makeUrl(),function(doc){var disp=false,img=$('#img',doc);if(typeof img.attr('src')!=='undefined'&&MCR.TMP.page<=2){MCR.TMP.list.push(img.attr('src'));if(MCR.TMP.page==1){doPage1(doc);}
MCR.TMP.page++;mainLoop();}else{updateDisplay();}});};MCR.Do.displayStatus('Loading Ch. '+MCR.Global.request.chapter+'...');MCR.TMP.list=[];MCR.TMP.page=1;mainLoop();},selectedChapter:function(){MCR.Global.request.chapter=$('#chapters').val();MCR.Get.chapter();},previousChapter:function(){var chi=MCR.Global.manga.chapterOrder.indexOf(MCR.Global.request.chapter);if(MCR.Global.manga.chapterOrder[chi-1]){MCR.Global.request.chapter=MCR.Global.manga.chapterOrder[chi-1];MCR.Get.chapter();}else{MCR.Do.displayStatus('This is the first Chapter');}},nextChapter:function(){var chi=MCR.Global.manga.chapterOrder.indexOf(MCR.Global.request.chapter);if(MCR.Global.manga.chapterOrder[chi+1]){MCR.Global.request.chapter=MCR.Global.manga.chapterOrder[chi+1];MCR.Get.chapter();}else{MCR.Do.displayStatus('This is the last Chapter');}}}};window.MCR=MCR;})(window);$(function(){function omvKeyPressed(){}
