...
self.loadChapter();};this.setChapter=function(n){chapter=n-1;};this.getChapter=function(){return chapter;};this.loadChapter=function(n){var chn=n||chapter;chapters[numbers[chn]].data.get();};var request=function(url,callback){$.ajax({type:"GET",url:url,success:function(data){callback((new DOMParser()).parseFromString(data,"text/xml"));},error:function(){callback((new DOMParser()).parseFromString("<html><head></head><body></body></html>","text/xml"));}});};};window.Ç.Manga=Manga;})(window);"use strict";(function(window){var Ui=function(){var manga=title=chaptername="",artist=author=rstatus="",chapterno=release=0,description=cover="";var navs=['info','options','hotkeys'];var buttons=['previous','next','reload','home','info','options','permalink','hotkeys'];var options=[];var hotkeys={'next':68,'previous':65,'home':87,'reload':83,'info':73,'options':79,'hotkeys':72};var self=this;var k;var def=function(val,def){if(val===""||val===undefined){return def;}
return val;};this.setStatus=function(message){message=message||'';var msg=message+(message==''?'':' / ');$('#status').text(message);$('title').text(msg+chapterno+chaptername+" / "+title);};this.updateURL=function(){if(Modernizr.history){history.pushState({manga:manga,chapter:chapterno},$('title').text(),"http://www.mangareader.net/"+manga+"/"+chapterno);}
$('#button-permalink').attr('href',"http://www.mangareader.net/"+manga+"/"+chapterno);};this.setInfo=function(_manga,_chapterno,_chaptername,_artist,_author,_release,_rstatus,_description,_cover){manga=def(_manga,manga);chapterno=def(_chapterno+1,chapterno);chaptername=def(_chaptername,chaptername);artist=def(_artist,artist);author=def(_author,author);release=def(_release,release);rstatus=def(_rstatus,rstatus);description=def(_description,description);cover=def(_cover,cover);title=manga.replace(/-/g,' ').replace(/^([a-z])|\s+([a-z])/g,function($1){return $1.toUpperCase();});if(chaptername!==""){chaptername=": "+chaptername;}
var $info=$('nav#info');$('h1',$info).text(title);$('h2',$info).text(chapterno+chaptername);$('date',$info).text(release);$('#stat',$info).text(rstatus);$('#cover',$info).attr('src',cover);$('#description',$info).text(description);if(author==artist){$('#arthor').html('Story / Art by <strong>'+author+'</strong>.');}else{if(artist==''){artist=author;}
...
ui.display(pages);if(gotinfo){ui.setInfo(false,manga.getChapter(),manga.chapters()[manga.getChapter()].name);ui.setStatus('');ui.updateURL();}}).bind("gotinfo",function(e,page){manga.addChapter(c);gotinfo=true;ui.setInfo(manga.info().name,manga.getChapter(),manga.chapters()[manga.getChapter()].name,manga.info().artist,manga.info().author,manga.info().pubdate,manga.info().status,manga.info().description,manga.info().cover);ui.setStatus('');ui.updateURL();}).bind("gotpage",function(e,page,chap){ui.setStatus('Loading Ch. '+(+chap.num())+' ('+page+')');}).bind("firstchapter",function(){ui.setStatus('This is the first chapter');}).bind("lastchapter",function(){ui.setStatus('This is the last chapter');}).bind("keyreload hitreload",function(){window.location=$('#button-permalink').attr('href');}).bind("keyprevious hitprevious",function(){manga.previousChapter();}).bind("keynext hitnext",function(){manga.nextChapter();});for(var n in options){ui.addOption(options[n]);(function(name){$(window).bind("keyopt"+name+" hitopt"+name,function(){ui["do_"+name](ui.getOption(name));console.log('Got opt: '+name);});})(options[n]['name']);}
