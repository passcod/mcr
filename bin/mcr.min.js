...
this.info=function(){return info;};this.chapters=function(){return chapters;};this.addChapter=function(chap){chapters[chap.num()].data=chap;};this.nextChapter=function(){if(chapter<numbers.length){chapter++;}else{$(window).trigger('lastchapter');}
self.loadChapter();};this.previousChapter=function(){if(chapter>=0){chapter--;}else{$(window).trigger('firstchapter');}
self.loadChapter();};this.setChapter=function(n){chapter=n-1;};this.getChapter=function(){return chapter;};this.loadChapter=function(n){var chn=n||chapter;chapters[numbers[chn]].data.get();};var request=function(url,callback){$.ajax({type:"GET",url:url,success:function(data){callback((new DOMParser()).parseFromString(data,"text/xml"));},error:function(){callback((new DOMParser()).parseFromString("<html><head></head><body></body></html>","text/xml"));}});};};window.Ç.Manga=Manga;})(window);"use strict";(function(window){var Ui=function(){var manga=chapterno=chaptername=artist=author=release=rstatus="";var navs=['info','options','hotkeys','theme','favs','mark'];var buttons=['previous','next','reload','home','info','options','permalink','hotkeys','theme','favs','mark'];var hotkeys={'next':68,'previous':65,'home':87,'reload':83,'info':73,'options':79,'hotkeys':72,'theme':84,'favs':70,'mark':66,'theme1':49,'theme2':50,'theme3':51,'favadd':-1,'favdel':-1,'favtog':-1,'markadd':-1,'markdel':-1,'marktog':-1};var self=this;var k;this.setStatus=function(message){message=message||'';var title=message+(message==''?'':' / ');$('#status').text(message);$('title').text(title+chapterno+": "+chaptername+" / "+manga);};this.setInfo=function(_manga,_chapterno,_chaptername,_artist,_author,_release,_rstatus){manga=_manga||manga;chapterno=_chapterno||chapterno;chaptername=_chaptername||chaptername;artist=_artist||artist;author=_author||author;release=_release||release;rstatus=_rstatus||rstatus;var $info=$('nav#info');$('h1',$info).text(manga);$('h2',$info).text(chapterno+": "+chaptername);$('date',$info).text(release);$('#stat',$info).text(rstatus);if(artist==''){artist=author;}
...
return colour;}
var ruls={columnRule:colorStyle(border)};ruls[Modernizr.prefixed('columnRuleColor')]=colorStyle(border);$('.has-bg').css('background-color',colorStyle(background));$('.has-border').css('border-color',colorStyle(border));$('.has-rule').css(ruls);$('body').css('color',colorStyle(text));};this.display=function(pages){var p,$p=$('article ul');$('li',$p).remove();for(var p in pages){$p.append('<li><img src="'+pages[p]+'" /></li>');}};this.updateURL=function(mng,chp){if(Modernizr.history){history.pushState({manga:mng,chapter:chp+1},$('title').text(),"http://www.mangareader.net/"+mng+"/"+(chp+1));}
$('#button-permalink').attr('href',"http://www.mangareader.net/"+mng+"/"+(chp+1));};$('head').append('<style>'+Ç.data.css+'</style>');$('body > *').not('script, #container').remove();$('body').append(Ç.data.html).add('header nav').addClass('has-bg');$('header nav').addClass('has-border');$('#version').text(Ç.data.version);$('nav#main').hover(function(){self.showMain();$(this).data('hover',true);},function(){self.hideMain();$(this).data('hover',false);}).data('hover',false);for(k in navs){(function(name,that){$(window).bind("key"+name+" hit"+name,function(){if($('nav#'+name).is(':hidden')){that.showNav(name);that.showMain();}else{that.hideNav(name);if($('nav#main').data('hover')===false){that.hideMain();}}});})(navs[k],this);}
