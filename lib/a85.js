var Ascii85 = {}; Ascii85 = {
  /* Ascii85 with Unicode support */
  
  c: function(input, length, result){
  ...
  encodeArray: function(input){
	  ...
	  Ascii85.c(input, length, result);
	  if(reminder){
		  var t = input.slice(length);
		  while(t.length < 4){ t.push(0); }
		  Ascii85.c(t, 4, result);
		  var x = result.pop();
		  if(x == "z"){ x = "!!!!!"; }
  ...
  encode: function (str) {
    ...
    for ( i in arr ) {
      cod.push(arr[i].charCodeAt(0));
    }
    
    return "<~"+Ascii85.encodeArray(cod)+"~>";
  },
  ...
  decode: function (str) {
    ...
    var arr = Ascii85.decodeArray(str),
      ret = [],
      i, c;
